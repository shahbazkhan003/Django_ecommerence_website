...
class add_to_cart_api(generics.CreateAPIView):
...
class show_cart(generics.ListAPIView):
    permission_classes([IsAuthenticated])
    def list(self, request, *args, **kwargs):
        if request.user.is_authenticated:
            user = request.user
...
class plus_cart_api(generics.RetrieveAPIView):
    @permission_classes([IsAuthenticated])
    def retrieve(self, request, *args, **kwargs):
...
class minus_cart_api(generics.RetrieveAPIView):
    @permission_classes([IsAuthenticated])
    def retrieve(self, request, *args, **kwargs):
        if request.method == "GET":
            try:
                ...
                return Response({"error": "Cart item not found for this user and product."}, status=404)
class remove_cart_api(generics.DestroyAPIView):
    @permission_classes([IsAuthenticated])
...
class orders_api(generics.ListAPIView):
    @permission_classes([IsAuthenticated]) 
    def list(self, request):
...
class checkout_api(generics.ListAPIView):
    @permission_classes([IsAuthenticated])
    def list(self, request):
        user = request.user
...
class payment_done_api(generics.CreateAPIView):
    @permission_classes([IsAuthenticated])
    def create(self, request):
        user = request.user
...
class search_api(generics.RetrieveAPIView):
    def retrieve(self, request, *args, **kwargs):
        pk = self.kwargs['pk']     
